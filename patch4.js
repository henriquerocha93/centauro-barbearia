...
code = code.replace('const totalProducts = (apt.products || []).reduce((sum, p) => sum + (p.price * p.qty), 0);', 'const totalProducts = (apt.products || []).reduce((sum, p) => sum + ((parseFloat(p.price)||0) * (p.qty||1)), 0);');
